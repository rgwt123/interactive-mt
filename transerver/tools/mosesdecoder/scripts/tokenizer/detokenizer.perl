...
sub charIsCJK {
    ...
    # fix chinese biaodian by twang
    return 1 if (&between_hexes($codepoint,'FF0C','FF0C')); #，
    return 1 if (&between_hexes($codepoint,'FF01','FF01')); #！
    return 1 if (&between_hexes($codepoint,'FF1F','FF1F')); #？
    return 1 if (&between_hexes($codepoint,'FF1B','FF1B')); #；
    return 1 if (&between_hexes($codepoint,'FF1A','FF1A')); #：
