...
class TransformerDecoder(nn.Module):
    ...
    def generate(self, encoded, max_len=200, sample=False, temperature=None, prefix_tokens=None):
        """
        ...
        """
        if self.beam_size > 0:
            return self.generate_beam(encoded, self.beam_size, max_len, sample, temperature, prefix_tokens)
        else:
            logger.error('Not implemented! Beam size need a value > 0 !')
            exit(0)

    def generate_beam(self, encoded, beam_size=4, max_len=100, sample=False, temperature=None, prefix_tokens=None):
        """
        Generate a sentence from a given initial state.
        Input:
            - FloatTensor of size (batch_size, hidden_dim) representing
        ...
        if is_cuda:
            x_len = x_len.cuda()
        results = generator.generate(x_len, encoded, prefix_tokens=prefix_tokens)
